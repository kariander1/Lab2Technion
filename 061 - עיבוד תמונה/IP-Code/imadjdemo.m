...
ud.hImgPop = uicontrol(Ctl, ...
   ...
   'Enable','on', ...
   'String','Circuit|Tire|Pepper|Rice|Pout|Trees|Saturn|Quarter|Glass|Lifting Body|Other', ...
   'Tag','ImagesPop',...
...
bins = (1:n)';
axes(ax)
if (ax.Position(1))>100
    titl = 'Adjusted Histogram';
    clr = [0 0 1];
else
    titl = 'Histogram';
    clr = [0 0 0];
end
bar(bins,cnts,'FaceColor',clr,'EdgeColor',clr);
h = get(ax,'children');
...
case 'Glass',
   ...
   img = glass; 
case 'Browse',
    [FileName,PathName] = uigetfile({'*.jpg;*.tif;*.png;*.bmp','All Image Files'; '*.*','All Files' },'Please choose an image');
    cd(PathName);
    img = double(imresize(rgb2gray(imread(FileName)),[128 NaN]));
otherwise 
